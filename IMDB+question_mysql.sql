...
GROUP BY movie_id HAVING Count(DISTINCT genre) = 1) SELECT Count(*) AS movies_with_one_genre FROM movies_with_one_genre;
...
( SELECT genre, COUNT(movie_id), DENSE_RANK() OVER(ORDER BY COUNT(movie_id) DESC) genre_rank FROM genre GROUP BY genre )
...
FROM movie m INNER JOIN ratings r ON m.id = r.movie_id WHERE r.avg_rating > 8 AND m.production_company IS NOT NULL
...
SELECT 
    COUNT(m.id) AS no_of_movie_released, r.median_rating
FROM
    movie AS m
        INNER JOIN
    ratings AS r ON m.id = r.movie_id
